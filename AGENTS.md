...
Keep the local Ollama daemon available on `http://localhost:11434` before running PHP integration tests or manual checks—the default provider relies on the `minimax-m2:cloud` model. Swap to OpenAI, Anthropic, Google Gemini, or OpenRouter via **Settings → AI Persona** when you need remote models; add your API key and override base/model fields if you target non-default regions.
...
The default Ollama provider runs locally and avoids transmitting content externally. When using OpenAI, Anthropic, Gemini, or OpenRouter adapters, store API keys securely, review data residency constraints, and document any organisation-wide policies for remote inference. OpenRouter exposes multiple upstream models—ensure you comply with each model’s usage policies when selecting identifiers.
...
- Override auth handling per integration via the `ai_persona_rest_permissions_check` filter (return `true` to allow or a `WP_Error` to deny).
...
- Chat widget now supports a persona switcher: enable it in the block inspector or pass JSON via the shortcode `persona_options` attribute to expose curated personas on the front-end.
- Remote provider adapters now include Google Gemini (`gemini-1.5-flash`) and OpenRouter (`openai/gpt-4o-mini` default). Update API keys and optional base URLs/models under Settings → AI Persona before routing traffic through them.
- Settings screen now ships with nav tabs (General, Analytics & Logging, Webhooks, Permissions, Automation Auth). Future documentation tabs can slot into the same helper without restructuring.
- Dynamic variables now surface WP_Query insights: tokens like `{{current_query.post_titles}}`, `{{current_query.search_terms}}`, and `{{current_term.name}}` let prompts react to archive/search contexts out of the box.
